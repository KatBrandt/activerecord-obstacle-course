...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
      # users = User.joins(:items).select(:name).where('items.id = ?', @item_8).order('users.name').distinct.pluck(:name)

     # cleaner syntax:
      users = User.joins(:items).select(:name).where(items: { id: @item_8 }).order(:name).distinct.pluck(:name)
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    names = Item.joins(:orders).select('items.name').where('orders.id = ?', Order.last).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
      items_for_user_3_third_order = Order.joins(:items).where('orders.user_id = ?', @user_3).distinct[2].items.pluck(:name)
